...
# exp tag
tag="sinc" # tag for managing experiments.
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    rawdir=raw
    # Generate the raw features
    for x in test dev train; do
        make_raw.sh --cmd "$train_cmd" --nj 32 --raw_config conf/raw.yaml --write_utt2num_frames true \
            data/${x} exp/make_raw/${x} ${rawdir}
    ...
    make_raw.sh --cmd "$train_cmd" --nj 32 --raw_config conf/raw.yaml --write_utt2num_frames true \
        data/${train_set} exp/make_raw/${train_set} ${rawdir}
