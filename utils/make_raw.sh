...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
  cat >&2 <<EOF
Usage: $0 [options] <data-dir> [<log-dir> [<raw-dir>] ]
 e.g.: $0 data/train
Note: <log-dir> defaults to <data-dir>/log, and
      <raw-dir> defaults to <data-dir>/data
Options:
  --raw-config <config-file>         # config passed to compute-raw-feats.py.
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_raw_${name}.JOB.log \
    ...
    compute-raw-feats.py $write_utt2dur_opt \
      --config=$raw_config ark:- ark:- \| \
  ...
  $cmd JOB=1:$nj $logdir/make_raw_${name}.JOB.log \
    compute-raw-feats.py $write_utt2dur_opt \
     --config=$raw_config scp,p:$logdir/wav.JOB.scp ark:- \| \
    copy-feats --compress=$compress $write_num_frames_opt ark:- \
     ark,scp:$rawdir/raw_raw_$name.JOB.ark,$rawdir/raw_raw_$name.JOB.scp \
     || exit 1;
fi
...
if [ -f $logdir/.error.$name ]; then
  echo "$0: Error producing raw features for $name:"
...
# Store frame_shift and raw_config along with features.
frame_shift=$(perl -ne 'if (/^--frame-shift=(\d+)/) {
                          printf "%.3f", 0.001 * $1; exit; }' $raw_config)
echo ${frame_shift:-'0.01'} > $data/frame_shift
mkdir -p $data/conf && cp $raw_config $data/conf/raw.conf || exit 1
...
echo "$0: Succeeded creating raw features for $name"
